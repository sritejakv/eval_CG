...
public class ResourceReader {
    ...
    private static void getFilesRecursively(String directory, List<File> files){
        ...
        if (dir.isDirectory()){
            for (File f: dir.listFiles()){
                if (f.isFile() && f.getName().endsWith(Constants.DOT_SUFFIX)) {
                    files.add(f);
     ...
     * @param benchmarkCategory sub folder inside framework folder name
     * @return list of files found
     */
    public static List<File> getFilesInResource(String framework, String benchmarkCategory){
        File[] folderList = new File(Constants.RESOURCES_PATH + framework).listFiles();
        ArrayList<File> fileList = new ArrayList<>();
        for (File category: folderList){
            if (category.getName().contains(benchmarkCategory)){
                getFilesRecursively(category.getAbsolutePath(), fileList);
            }
        }
        return fileList;
    }
    ...
    public static void main(String...args) {
        System.out.println(ResourceReader.getResource("pyan", "eval_bark.dot"));
        System.out.println(ResourceReader.getFilesInResource("pycallgraph", "code_generation"));
    }
}
